...
import { ChromaClient, GoogleGenerativeAiEmbeddingFunction, OpenAIEmbeddingFunction } from 'chromadb';
...
class ChromaService {
	...
	static init(key) {
		this.client = new ChromaClient({
			...
			auth: { provider: 'basic', credentials: process.env.CHROMA_SERVER_CREDENTIALS || key },
		});
	}

	static createEmbedder = (integration, model) => {
		let embeddingFunction;
		switch(integration.toLowerCase()) {
			case 'google':
				embeddingFunction = new GoogleGenerativeAiEmbeddingFunction({
	 ...
	 * @param {object} [props={}] - Additional properties for the collection.
	 * @returns {Promise<object>} - The created collection.
	 */
	static async createOrGetCollection(name, props = {}) {
		try {
	 ...
	 */
	static async getCollection(name, props = {}) {
		return await this.client.getCollection({
			name: name,
			...props,
		});
	 ...
	 */
	static async generateEmbeddings(texts, integration = 'google', model = '') {
		let embeddingFunction;
		switch(integration) {
			case 'google':
				embeddingFunction = new GoogleGenerativeAiEmbeddingFunction({
				...
				try {
					...
					throw new Error('Failed to generate embeddings using Google: ' + error.message);
				}
			case 'openai':
				embeddingFunction = new OpenAIEmbeddingFunction({
					openai_api_key: process.env.OPENAI_API_KEY,
